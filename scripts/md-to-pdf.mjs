...
function getTopicDirs(categoryDir) {
  return readdirSync(categoryDir)
    .filter(f => {
      const fullPath = join(categoryDir, f);
      try {
        return readdirSync(fullPath).length > 0 && isTopicDir(fullPath);
      } catch {
        return false;
      }
    })
...
async function main() {
  ...
  // Mode 1: --all (generate individual PDF per topic)
  if (args.includes('--all')) {
    ...
    for (const category of CATEGORIES) {
      ...
      for (const topic of topicDirs) {
        const topicPath = join(categoryDir, topic);
        const files = getMdFiles(topicPath);

        if (files.length === 0) continue;

        const sections = files.map(f => {
          const filePath = join(topicPath, f);
        ...
        const title = topic;
        const fullHtml = buildHtml(sections, title);
    ...
    console.log(`\nDone! Generated ${totalPdfs} PDFs in output/`);
  ...
  if (folderIdx !== -1 && args[folderIdx + 1]) {
    const folderPath = resolve(ROOT, args[folderIdx + 1]);
    if (!existsSync(folderPath)) {
      console.error(`Folder not found: ${args[folderIdx + 1]}`);
      await highlighter.dispose();
      process.exit(1);
    }
  ...
  if (sections.length === 1) {
    ...
    const fullHtml = buildHtml(sections, 'Notes');
    const outputPath = join(OUTPUT_DIR, 'Notes.pdf');
    await generatePdf(fullHtml, outputPath);
  }
